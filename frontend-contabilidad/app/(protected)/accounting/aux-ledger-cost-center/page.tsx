...
import {
  buildAuxLedgerCostCenterExportUrl,
  fetchAuxLedgerByCostCenter,
  type AuxLedgerCostCenterQueryParams,
  type AuxLedgerCostCenterResponse,
  type AuxLedgerRow,
  type JournalStatus,
...
function buildFilters(params: {
  ...
  thirdPartyId: number | ''
  costCenterId: string
}): AuxLedgerCostCenterQueryParams | null {
  const costCenterId = sanitizeNumber(params.costCenterId)
  if (costCenterId === undefined) return null
  const filters: AuxLedgerCostCenterQueryParams = {
    from: params.from,
    to: params.to,
    costCenterId,
  }
...
export default function AuxLedgerCostCenterPage() {
  ...
  const [report, setReport] = useState<AuxLedgerCostCenterResponse | null>(null)
  ...
  const filters = useMemo<AuxLedgerCostCenterQueryParams | null>(() =>
    buildFilters({
      from,
      to,
      status,
  ...
  const exportCsvUrl = useMemo(() => {
    if (!filters) return '#'
    return buildAuxLedgerCostCenterExportUrl({ ...filters, format: 'csv' })
  }, [filters])

  const exportXlsxUrl = useMemo(() => {
    if (!filters) return '#'
    return buildAuxLedgerCostCenterExportUrl({ ...filters, format: 'xlsx' })
  }, [filters])

  async function loadReport() {
    if (!filters) {
      setError('Ingresa el ID del centro de costo para consultar el auxiliar.')
      setReport(null)
      return
    }
    ...
    setError(null)
    try {
  const data = await fetchAuxLedgerByCostCenter(filters)
      setReport(data)
    } catch (err: any) {
      setReport(null)
      setError(err?.response?.data?.message || err?.message || 'No se pudo cargar el auxiliar por centro de costo')
    } finally {
    ...
    <Protected>
      ...
      <main className="container mx-auto p-6">
        <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl font-semibold tracking-tight">Auxiliar por centro de costo</h1>
            <p className="text-sm text-gray-500">
              Analiza los movimientos asociados a un centro de costo y sus cuentas relacionadas.
            </p>
          </div>
        ...
        >
          ...
          <div className="md:col-span-2">
            <label className="mb-1 block text-sm font-medium">Centro de costo (ID requerido)</label>
            <input
              type="number"
              className="input input-bordered w-full rounded-xl"
              placeholder="Ej: 100"
            ...
            <p className="mt-1 text-xs text-gray-500">
              Ingresa el identificador numérico del centro de costo configurado en el backend.
            </p>
          </div>

          <div className="md:col-span-2">
            <label className="mb-1 block text-sm font-medium">Tercero (opcional)</label>
            <SearchSelect
        ...
        {report && (
          <div className="mb-5 grid grid-cols-1 gap-4 md:grid-cols-4">
            <div className="rounded-2xl border bg-white p-4 shadow-sm md:col-span-2">
              ...
              <p className="text-lg font-semibold">{report.costCenter?.code ?? '—'}</p>
              {report.costCenter?.name && (
                <p className="text-sm text-gray-600">{report.costCenter.name}</p>
              )}
            </div>
              ...
              {!loadingReport && rows.length === 0 && (
                <tr>
                  <td colSpan={9} className="px-4 py-6 text-center text-sm text-gray-500">
                    {filters ? 'No hay movimientos para el centro de costo indicado.' : 'Indica el ID del centro de costo para ver sus movimientos.'}
                  </td>
                </tr>
              )}
            </tbody>
