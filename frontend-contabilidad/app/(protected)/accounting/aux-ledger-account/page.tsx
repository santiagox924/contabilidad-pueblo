...
import {
  buildAuxLedgerAccountExportUrl,
  fetchAuxLedgerByAccount,
  type AuxLedgerAccountQueryParams,
  type AuxLedgerAccountResponse,
  type AuxLedgerRow,
  type JournalStatus,
...
function buildFilters(params: {
  ...
  costCenterId: string
  accountCode: string
}): AuxLedgerAccountQueryParams {
  const filters: AuxLedgerAccountQueryParams = {
    from: params.from,
    to: params.to,
    accountCode: params.accountCode.trim(),
  }
  if (params.status) filters.status = params.status
  if (params.journalCode.trim()) filters.journalCode = params.journalCode.trim().toUpperCase()
  if (typeof params.thirdPartyId === 'number') filters.thirdPartyId = params.thirdPartyId
  const costCenterId = sanitizeNumber(params.costCenterId)
  if (costCenterId !== undefined) filters.costCenterId = costCenterId
  return filters
}

export default function AuxLedgerAccountPage() {
  ...
  const [report, setReport] = useState<AuxLedgerAccountResponse | null>(null)
  ...
  const filters = useMemo<AuxLedgerAccountQueryParams | null>(() => {
    if (!selectedAccountCode.trim()) return null
    return buildFilters({
      from,
      to,
      status,
  ...
  }, [from, to, status, selectedJournalCode, selectedThirdPartyId, costCenterId, selectedAccountCode])

  const exportCsvUrl = useMemo(() => {
    if (!filters) return '#'
    return buildAuxLedgerAccountExportUrl({ ...filters, format: 'csv' })
  }, [filters])

  const exportXlsxUrl = useMemo(() => {
    if (!filters) return '#'
    return buildAuxLedgerAccountExportUrl({ ...filters, format: 'xlsx' })
  }, [filters])

  async function loadReport() {
    if (!filters) {
      setError('Selecciona una cuenta contable antes de consultar el auxiliar.')
      setReport(null)
      return
    }
    ...
    setError(null)
    try {
      const data = await fetchAuxLedgerByAccount(filters)
      setReport(data)
    } catch (err: any) {
      setReport(null)
      setError(err?.response?.data?.message || err?.message || 'No se pudo cargar el auxiliar por cuenta')
    } finally {
    ...
    <Protected>
      ...
      <main className="container mx-auto p-6">
        <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl font-semibold tracking-tight">Auxiliar por cuenta</h1>
            <p className="text-sm text-gray-500">
              Consulta el movimiento contable detallado de una cuenta, incluyendo saldo inicial y final.
            </p>
          </div>
        ...
        {report && (
          <div className="mb-5 grid grid-cols-1 gap-4 md:grid-cols-4">
            <div className="rounded-2xl border bg-white p-4 shadow-sm">
              ...
              <p className="text-lg font-semibold">{report.account.code}</p>
              <p className="text-sm text-gray-600">{report.account.name}</p>
            </div>
            <div className="rounded-2xl border bg-white p-4 shadow-sm">
              <p className="text-sm text-gray-500">Saldo inicial</p>
              <p className="text-2xl font-semibold">${fmtMoney(report.opening)}</p>
            </div>
            <div className="rounded-2xl border bg-white p-4 shadow-sm">
            ...
            <div className="rounded-2xl border bg-white p-4 shadow-sm">
              ...
              <p className="text-2xl font-semibold">${fmtMoney(report.closing)}</p>
            </div>
          </div>
        )}
              ...
              {!loadingReport && rows.length === 0 && (
                <tr>
                  <td colSpan={9} className="px-4 py-6 text-center text-sm text-gray-500">
                    {filters ? 'No hay movimientos para los filtros aplicados.' : 'Selecciona una cuenta para ver su auxiliar.'}
                  </td>
                </tr>
              )}
            </tbody>
            {!loadingReport && rows.length > 0 && (
              <tfoot className="bg-gray-50">
                <tr>
                  ...
                  <td className="px-3 py-2 text-sm text-right font-semibold">{fmtMoney(report?.closing ?? 0)}</td>
                </tr>
              </tfoot>
            )}
          </table>
