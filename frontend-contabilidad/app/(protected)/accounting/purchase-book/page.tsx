...
export default function PurchaseBookPage() {
  ...
  const [rows, setRows] = useState<PurchaseBookLine[]>([])
  ...
  async function load() {
    ...
    try {
      const res = await api.get<PurchaseBookLine[]>('/accounting/purchase-book', { params: { from, to, group } })
      setRows(Array.isArray(res.data) ? res.data : [])
    } catch (e: any) {
      setError(e?.response?.data?.message || e?.message || 'Error cargando libro de compras')
      setRows([])
  ...
  const exportUrl = useMemo(() => {
    ...
    return `/accounting/export/purchase-book.csv?${qs}`
  }, [from, to, group])
    ...
    <Protected>
      ...
      <main className="container mx-auto p-6">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-semibold tracking-tight">Libro de compras</h1>
            <p className="text-sm text-gray-500">Listado de compras por {group === 'day' ? 'día' : 'documento'}.</p>
          </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-2 text-left text-sm">Proveedor</th>
